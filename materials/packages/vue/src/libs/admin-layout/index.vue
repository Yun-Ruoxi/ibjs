<template>
  <div :class="[':ib: relative h-full', commonClass]" :style="cssVars">
    <div
      :id="isWrapperScroll ? scrollElId : undefined"
      :class="[
        ':ib: flex flex-col h-full',
        commonClass,
        scrollWrapperClass,
        { ':ib: overflow-y-auto': isWrapperScroll }
      ]"
    >
      <!-- 头部 -->
      <template v-if="showHeader">
        <header
          ...
          :class="[
            ...
            headerClass,
            headerLeftGapClass,
            { ':ib: absolute top-0 left-0 w-full': fixedHeaderAndTab }
          ]"
        >
        ...
        <div
          ...
          :class="[style['layout-header-placement'], ':ib: flex-shrink-0 overflow-hidden']"
        ></div>
      </template>
      ...
      <template v-if="showTab">
        <div
          ...
          :class="[
            ...
            { 'top-0!': !showHeader },
            leftGapClass,
            { ':ib: absolute left-0 w-full': fixedHeaderAndTab }
          ]"
        >
          <slot name="tab"></slot>
        </div>
        <div
          v-show="!fullContent && fixedHeaderAndTab"
          :class="[style['layout-tab-placement'], ':ib: flex-shrink-0 overflow-hidden']"
        ></div>
      </template>
      ...
      <template v-if="showSider">
        <aside
          v-show="!fullContent"
          :class="[
            ':ib: absolute left-0 top-0 h-full',
            commonClass,
            siderClass,
            siderPaddingClass,
      ...
      <template v-if="showMobileSider">
        <aside
          :class="[
            ':ib: absolute left-0 top-0 w-0 h-full bg-white',
            commonClass,
            mobileSiderClass,
        ...
        <div
          ...
          :class="[':ib: absolute left-0 top-0 w-full h-full bg-[rgba(0,0,0,0.2)]', style['layout-mobile-sider-mask']]"
      ...
      <main
        ...
        :class="[
          ':ib: flex flex-col flex-grow',
          commonClass,
          contentClass,
          leftGapClass,
          { ':ib: overflow-y-auto': isContentScroll }
        ]"
      >
        <slot></slot>
      </main>
      ...
      <template v-if="showFooter">
        <footer
          ...
          :class="[
            ...
            footerClass,
            footerLeftGapClass,
            { ':ib: absolute left-0 bottom-0 w-full': fixedFooter }
          ]"
        >
        ...
        <div
          ...
          :class="[style['layout-footer-placement'], ':ib: flex-shrink-0 overflow-hidden']"
        ></div>
      </template>
    </div>
  </div>
